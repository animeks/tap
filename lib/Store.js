...
function bind(zidni) {
    ...
    zidni.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    zidni.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    zidni.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    zidni.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   zidni.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    zidni.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    zidni.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    zidni.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    zidni.ev.on('chats.update', updates => {
    ...
    zidni.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    zidni.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**zidni.ev.on('messages.update', updates => {
    ...
    zidni.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    zidni.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    zidni.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (zidni === null || zidni === void 0 ? void 0 : zidni.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id, zidni) => {
            var _a
            let message = (_a = messages[jid]) === null || _a === void 0 ? void 0 : _a.get(id)
            if (!message) {
                message = await (zidni === null || zidni === void 0 ? void 0 : zidni.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (zidni === null || zidni === void 0 ? void 0 : zidni.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return zidni === null || zidni === void 0 ? void 0 : zidni.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (zidni === null || zidni === void 0 ? void 0 : zidni.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (zidni === null || zidni === void 0 ? void 0 : zidni.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (zidni === null || zidni === void 0 ? void 0 : zidni.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (zidni === null || zidni === void 0 ? void 0 : zidni.messageInfo(remoteJid, id))
                if (msg) {
